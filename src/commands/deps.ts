...
import { projectUsing, getDependencyGraph, invalidateDependencyCache } from "src/ast";
import { AsOption } from "src/cli";
import { msg } from "src/utils";
import { traverseDependencies, findSymbolsByPattern } from "src/ast/dependency-graph";
import { handleSymbolSelection } from "src/utils/interactive";
import { DependencyNode, DependencyGraph } from "~/types/dependency";
...
function printDependencyTree(
  ...
  for (const dep of traversal.dependencies) {
    if (dep.depth > maxDepth) continue;
